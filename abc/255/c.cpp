...
int main() {
    ...
    //場合分け(d = 0 の場合を含む)
    if (x <= smin) {cout << abs(smin - x) << endl; return 0;}
    if (smax <= x) {cout << abs(x - smax) << endl; return 0;}
    
    //それ以外。
    long long rem = (x + d - a) % d;
    if (0 < d) cout << min(rem, d-rem) << endl;
    else cout << min(-rem, -(d-rem)) << endl;
