...
int main() {
    ...
    long long MAX = -1'000'000'000'000'000LL;
    vector<vector<long long>> dp(n+1, vector<long long>(w+1, MAX));
    dp[0][0] = 0;
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j <= w; j++) {
            if (dp[i-1][j] >= 0) dp[i][j] = dp[i-1][j];
            if (j - ws[i-1] >= 0 && dp[i-1][j - ws[i-1]] >= 0) dp[i][j] = max(dp[i-1][j - ws[i-1]] + vs[i-1], dp[i][j]);  
        }
    }
