...
int main() {
    ...
    int n, d; cin >> n >> d;
    vector<int> day(n), money(n);
    set<pair<int, int>> s;
    for (int i = 0; i < n; i++) cin >> day[i] >> money[i];

    vector<bool> takenJob(n, false);
    int ans = 0;
    for (int currentDay = 1; currentDay <= d; currentDay++) {
        int maxMoney = 0;
        int jobId = -1;
        for (int j = 0; j < n; j++) {
            if (takenJob[j]) continue;
            if (day[j] <= currentDay && money[j] > maxMoney) {
                maxMoney = money[j];
                jobId = j;
            }
        }
