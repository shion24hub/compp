...
int main() {
    ...
    vector<vector<long long>> dp(n+1, vector<long long>(w+1, MIN));
    dp[0][0] = 0;
    for (int i = 1; i < n+1; i++) {
        for (int j = 0; j < w+1; j++) {
            if (dp[i-1][j] > MIN) dp[i][j] = dp[i-1][j];
            if (j - ws[i-1] >= 0 && dp[i-1][j - ws[i-1]] > MIN) {
                if (dp[i][j] > MIN) dp[i][j] = max(dp[i][j], dp[i-1][j - ws[i-1]] + vs[i-1]);  
                else  dp[i][j] = dp[i-1][j - ws[i-1]] + vs[i-1];
            }
        }
    }
